...
function detectPacked(source) {
  const pattern = /eval\s*\(\s*function\s*\(\s*p\s*,\s*a\s*,\s*c\s*,\s*k\s*,\s*e\s*/;
...
function unpackPacked(source) {
  try {
    const argsRegex = /}\s*\('(.*)',\s*(.*?),\s*(\d+),\s*'(.*?)'\.split\('\|'\)/s;
    ...
    function lookup(word) {
      const index = radix === 36 ? parseInt(word, 36) : parseInt(word, radix);
      return index < symtab.length && symtab[index] ? symtab[index] : word;
    }
