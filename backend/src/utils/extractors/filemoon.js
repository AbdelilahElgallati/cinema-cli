  ...
  for (const match of matches) {
    ...
    if (unpacker.detect()) {
      ...
      if (unpacked) {
        ...
        console.log(`[getPackedData] Failed to unpack code #${matchCount}`);
      }
    } else {
      console.log(`[getPackedData] Code #${matchCount} not detected as packed`);
    }
  }

  // Try to extract FileMoon custom obfuscation
  // Look for patterns like: var K='...'...split("").reduce(...)
  const customObfuscationRegex = /var\s+\w+\s*=\s*'[^']{1000,}'.*?\.split\(""\)\.reduce\(/gs;
  const customMatches = html.matchAll(customObfuscationRegex);

  for (const match of customMatches) {
    console.log(`[getPackedData] Found custom obfuscation, attempting to extract...`);
    // Try to find sources or file URLs in the obfuscated code
    // The sources are often in the format: sources:[{file:"URL"}]
    const sourceInObfuscated = match[0].match(/sources.*?\[.*?file.*?["']([^"']+)["']/);
    if (sourceInObfuscated) {
      console.log(`[getPackedData] Found source in obfuscated code:`, sourceInObfuscated[1]);
  ...
  try {
    ...
    if (mediaId.includes('$$')) {
      ...
      try {
        ...
        console.log('[extract_filemoon] Invalid referer URL:', e.message);
        referer = false;
      }
    }
    ...
    if (mediaId.includes('/')) {
      ...
      console.log('[extract_filemoon] Media ID had slash, extracted:', oldMediaId, '->', mediaId);
    }
    ...
    console.log('[extract_filemoon] Request headers:', JSON.stringify(headers, null, 2));

    // First request
    console.log('[extract_filemoon] Making first request...');
    let response = await fetch(webUrl, { headers });
    console.log('[extract_filemoon] Response status:', response.status, response.statusText);
    ...
    if (
      ...
      html.includes('<h1>This video cannot be watched under this domain</h1>')
    ) {
      console.log('[extract_filemoon] Error page detected, trying /d/ endpoint');
    ...
    console.log('[extract_filemoon] Searching for video element with src...');
    const videoSrcMatch = html.match(/<video[^>]+src=["']([^"']+)["']/);
    if (videoSrcMatch && videoSrcMatch[1].includes('blob:')) {
      console.log('[extract_filemoon] Found blob URL in video tag, this is client-side generated');
      console.log('[extract_filemoon] Need to extract the actual m3u8/mp4 URL from JavaScript');
    }

    // Look for jwplayer setup which might contain the file URL
    console.log('[extract_filemoon] Searching for jwplayer setup...');
    const jwplayerSetup = html.match(/jwplayer\([^)]+\)\.setup\(({[\s\S]+?})\);/);
    if (jwplayerSetup) {
      console.log('[extract_filemoon] Found jwplayer setup');
      try {
        // Try to extract file URL from setup
        const fileMatch = jwplayerSetup[1].match(/["']?file["']?\s*:\s*["']([^"']+)["']/);
        if (fileMatch) {
          console.log('[extract_filemoon] Found file in jwplayer setup:', fileMatch[1]);
          ...
          console.log('[extract_filemoon] SUCCESS via jwplayer setup');
          return {
            file: fileMatch[1],
            type: 'mp4',
            headers: headers,
        ...
        console.log('[extract_filemoon] Error parsing jwplayer setup:', e.message);
      }
    }
    ...
    html += packedData;
    console.log('[extract_filemoon] Total HTML length after packed data:', html.length);
    ...
    if (postDataMatch) {
      ...
      console.log('[extract_filemoon] bMatch:', bMatch ? bMatch[1] : 'NOT FOUND');
      console.log(
        '[extract_filemoon] fileCodeMatch:',
        fileCodeMatch ? fileCodeMatch[1] : 'NOT FOUND'
      );
      console.log('[extract_filemoon] hashMatch:', hashMatch ? hashMatch[1] : 'NOT FOUND');
      ...
      console.log('[extract_filemoon] postData:', JSON.stringify(postData, null, 2));
      ...
      console.log('[extract_filemoon] Parsed edata:', JSON.stringify(edata, null, 2));
      ...
      console.log('[extract_filemoon] No postData found, trying fallback method');
      console.log('[extract_filemoon] Searching for sources pattern in HTML...');

      const sourcesMatch = html.match(/sources:\s*\[{\s*file:\s*["']([^"']+)["']/s);

      if (sourcesMatch) {
        console.log('[extract_filemoon] Found sources match:', sourcesMatch[1]);
        ...
        if (m3u8Match) {
          console.log('[extract_filemoon] Found m3u8 URL:', m3u8Match[1]);
