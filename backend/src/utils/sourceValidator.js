...
export async function validateSources(files, concurrency = 5, timeout = 5000) {
  ...
  for (let i = 0; i < files.length; i += concurrency) {
    ...
    const batchResults = await Promise.all(batch.map((file) => validateSource(file, timeout)));
    ...
    if (shouldDebug) {
      ...
      console.log(
        `[Source Validator] Checked ${totalChecked}/${files.length} sources, ${results.length} valid so far`
      );
    }
  }
  ...
  if (shouldDebug) {
    console.log(
      `[Source Validator] Validation complete: ${results.length}/${files.length} sources are valid`
    );
  }
