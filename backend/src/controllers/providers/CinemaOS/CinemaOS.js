...
export async function getCinemaOS(params) {
  ...
  try {
    ...
    if (shouldDebug) {
      console.log('CinemaOS: Extracted - Title:', title, 'Year:', releaseYear, 'IMDb:', imdbId);
    }
    ...
    if (shouldDebug) {
      console.log('CinemaOS: Encrypted response received:', encResponse ? 'YES' : 'NO');
    }
    ...
    const validEntries = Object.values(sources).filter((v) => v && typeof v === 'object' && v.url);
