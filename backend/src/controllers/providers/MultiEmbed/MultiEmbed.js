...
export async function getMultiembed(params) {
  ...
  try {
    ...
    if (!vipSource.length) throw new Error('No VIP source (B/S) found with valid data-id');
    ...
    if (!iframeUrl || iframeUrl.trim() === '') {
      throw new Error('Iframe src is empty — server may need JS init or different server');
    }
