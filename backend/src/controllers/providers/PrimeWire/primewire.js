  ...
  try {
    ...
    for (const item of urls) {
      try {
        ...
        console.error(`[Primewire] Skipping failed link ${item.idx}:`, err.message);
...
BlowfishCipher.prototype = {
  ...
  encryptECB: function (plaintext) {
    ...
    for (let blockIndex = 0; blockIndex < blockCount; blockIndex++) {
      ...
      ciphertext += this.num2block32(leftBlock) + this.num2block32(rightBlock);
    }
    return ciphertext;
  },
  ...
  decryptECB: function (ciphertext) {
    ...
    for (let blockIndex = 0; blockIndex < blockCount; blockIndex++) {
      ...
      plaintext += this.num2block32(leftBlock) + this.num2block32(rightBlock);
    }
  ...
  decryptCBC: function (ciphertext, initVector) {
    ...
    for (let blockIndex = 0; blockIndex < blockCount; blockIndex++) {
      ...
      plaintext += this.num2block32(leftPlain) + this.num2block32(rightPlain);
    }
    return this.utf8Encode(plaintext);
  },
  ...
  encipherBlock: function (leftHalf, rightHalf) {
    ...
    for (let round = 0; round < 16; round++) {
      ...
      leftHalf = rightHalf = this.xor(this.blowfishF(leftHalf), rightHalf);
      rightHalf = tempLeft;
    }
  ...
  generateSubkeys: function (password) {
    ...
    for (let pIndex = 0; pIndex < 18; pIndex++) {
      for (let byteCount = 4; byteCount > 0; byteCount--) {
        keyData = this.fixNegative((keyData << 8) | password.charCodeAt(keyIndex));
    ...
    for (let pIndex = 0; pIndex < 18; pIndex += 2) {
      encryptionInput = this.encipherBlock(encryptionInput[0], encryptionInput[1]);
...
async function fromPrimewireToProvider(primwireObject) {
  ...
  try {
    ...
    const scriptTags = response.data.match(/<script[^>]*src="[^"]*"[^>]*>/g);
    if (scriptTags) {
      // console.log(
      //     `[Primewire Debug] Found ${scriptTags.length} script tags`
      // );
      const appJsTags = scriptTags.filter((tag) => tag.includes('/js/app-'));
      // console.log(`[Primewire Debug] App.js tags:`, appJsTags);
    }

    // Step 2: Extract the app.js file version from the page
    let javascriptfile = response.data.match(/<script[^>]*src="\/js\/app-([^"?]+)\.js[^"]*"/);

    if (!javascriptfile) {
      javascriptfile = response.data.match(/<script[^>]*src="\/js\/app-([^"?]+)/);
    }
    ...
    if (!javascriptfile || !javascriptfile[1]) {
      console.error(`[Primewire] Could not find app.js reference for idx=${primwireObject.idx}`);
      // console.log(
    ...
    if (!token) {
      token = jsfiledata.data.match(/useEffect[^\{]*\{[^\}]*var [^;]*;[^=]*="([^"]+)"/);
    }
    ...
    console.log(`[Primewire Debug] Fetching media URL: ${mediaUrl}?token=${token}`);
    ...
    } else if (typeof mediaobject.data === 'string' && mediaobject.data.startsWith('http')) {
    ...
    if (error.response) {
      console.error(`[Primewire] Response status: ${error.response.status}`);
      console.error(`[Primewire] Response data:`, error.response.data?.substring?.(0, 500));
    }
