...
async function generateVRF(movieId, userId) {
  try {
    ...
    const key = await webcrypto.subtle.importKey('raw', keyHash, { name: 'AES-CBC' }, false, [
      'encrypt',
    ]);
    ...
    const encrypted = await webcrypto.subtle.encrypt({ name: 'AES-CBC', iv: iv }, key, plaintext);
