...
from src.config import (
    console, BACKEND_URL, TMDB_API_KEY,
    HISTORY_FILE, FAVORITES_FILE, PLAYBACK_FILE, SETTINGS_FILE,
    PRIMARY, SECONDARY, ACCENT, TEXT, SUCCESS, WARNING
)
...
class CinemaCLI:
    ...
    #         # Add navigation controls
    #         if data.get("total_pages", 1) > page:
    #             results.append({"id": "next_page", "name": "➡️ Next Page", "special": True})
    #         if page > 1:
    #             results.insert(0, {"id": "prev_page", "name": "⬅️ Previous Page", "special": True})
    ...
    def handle_genres(self):
        ...
        app = Application(
            ...
            key_bindings=kb,
            style=Style.from_dict({"selected": f"bg:{ACCENT} fg:#ffffff bold", "item": f"{TEXT}"}),
        )
        genre = app.run()
    ...
    def handle_settings(self):
        ...
        console.print(f"[bold {TEXT}]3. Movie Filename Template:[/bold {TEXT}] {self.settings.get('filename_template')}")
        console.print(f"[bold {TEXT}]4. TV Filename Template:[/bold {TEXT}] {self.settings.get('filename_template_tv')}")
        ...
        if choice == "1":
            ...
            console.print("[dim]Tokens: {title}, {year}, {season}, {episode}, {quality}, {provider}[/dim]")
    ...
    def handle_batch_download(self, media, season, episodes):
        ...
        for ep in selected_episodes:
               ...
               sel = selection_menu(files, f"Sources - {title}", show_details=False, formatter=fmt_src)
               ...
               template = self.settings.get("filename_template_tv", "{title}.S{season}E{episode}")
               filename = generate_filename(template, title, meta, selected_source)

               console.print(f"[green]Queuing download using {selected_source.get('provider')} ({selected_source.get('quality')})...[/green]")
               self.download_manager.add_task(selected_source.get("file"), filename, title, subtitles, selected_source.get("headers"), meta)
    ...
    def handle_sources(self, title, data, meta=None):
        ...
        while True:
            ...
            if act["value"] == "▶ Play":
                ...
                self.download_manager.add_task(selected.get("file"), filename, title, subtitles, selected.get("headers"), meta=meta)
                return False

    def start_player(self, url, title):
        print_header(title)
        console.print("1. ▶ Play with MPV\n2. ⬇ Download Video\n3. 🔗 Copy URL\n4. ⬅ Back")
        ...
        if choice == "1":
            ...
            filename = generate_filename(template, title, meta=None, selected={"provider": "direct", "quality": "auto"})
