...
def selection_menu(items, title, show_details=True, formatter=None, default_index=0):
    ...
    def get_formatted_text():
        ...
        res.append(('class:border', "─" * 60 + "\n"))
        res.append(('class:help', " [↑/↓] Navigate  [Enter] Select  [D] Batch Download  [F] Favorite  [B] Back  [Q] Quit "))
        return res
...
def multi_selection_menu(items, title, formatter=None):
    ...
    def get_formatted_text():
        ...
        res.append(('class:border', "─" * 60 + "\n"))
        res.append(('class:help', " [↑/↓] Navigate  [Space] Toggle  [A] Select All  [Enter] Confirm  [B/Q] Back "))
        return res
    ...
    app = Application(layout=PTLayout(Window(FormattedTextControl(get_formatted_text))), key_bindings=kb, style=style, full_screen=False)
    return app.run()
