    ...
    def report(self, provider: str, success: bool):
        ...
        with self._lock:
            ...
            if success:
                ...
                e["failures"] += 1
            e["score"] = (1 - self._EWA_ALPHA) * e["score"] + self._EWA_ALPHA * (
...
def verify_source(url, headers=None, timeout=8):  # NOSONAR
    ...
    url_origin = (
        f"{url.split('://')[0]}://{url.split('://')[1].split('/')[0]}" if "://" in url else None
    )
    if url_origin:
    ...
    try:
        ...
        if resp.status_code in [200, 206]:
            ...
            if any(
                x in content_type
                for x in ["video", "audio", "mpegurl", "octet-stream", "application"]
            ):
        ...
        if url_type in ["hls", "worker"] and resp.status_code in [
            200,
            206,
            301,
            302,
...
def select_working_source(
    sources, skip_validation=False, max_parallel=5, timeout_per_source=6
):  # NOSONAR
    """
    ...
    with ThreadPoolExecutor(max_workers=max_parallel) as executor:
        ...
        for future in as_completed(futures, timeout=timeout_per_source * 2):
            try:
                ...
                if is_valid:
                    console.print(
                        f"[green]  ✓ Source {idx+1}: {provider} [{quality}] - {reason}[/green]"
                    )
...
def select_multiple_working_sources(
    sources, count=3, skip_validation=False, max_parallel=8, timeout_per_source=5
):  # NOSONAR
    """
