...
def play_stream(url, title, subtitles=None, headers=None, meta=None, start_time=0):
    ...
    if shutil.which("yt-dlp"):
        ...
        if headers:
            ...
            if header_list:
                mpv_args.append(
                    f"--ytdl-raw-options=http-header-fields={','.join(header_list)}"
                )
    ...
    if subtitles:
        ar = [
            s for s in subtitles if s.get("lang", "").lower() in ["arabic", "ar", "ara"]
        ]
        if ar:
            ...
            try:
                ...
                sub_ext = (
                    "srt"
                    if ".srt" in sub_url
                    else ("vtt" if ".vtt" in sub_url else "srt")
                )
                base = (
