...
def download_stream(
    ...
    def run_download():
        ...
        if subtitles:
            ar = [
                s
                for s in subtitles
                if s.get("lang", "").lower() in ["arabic", "ar", "ara"]
            ]
            if ar:
                sub_url = ar[0]["url"]
        ...
        try:
                ...
                if not success and last_exc:
                    ...
                    if (
                        ...
                        try:
                            ...
                            ff_cmd = ["ffmpeg", "-y"]
                            if headers:
                                ua = headers.get("User-Agent") or headers.get(
                                    "user-agent"
                                )
                                if ua:
                                ...
                                if ref:
                                    ff_cmd.extend(["-headers", f"Referer: {ref}"])
            ...
            # 2. Try FFmpeg (Standard, faster than recording) - only if yt-dlp is not found
                ...
                if headers:
                    ...
                    ref = headers.get("Referer") or headers.get("referer")
                    if ref:
                        cmd.extend(["-headers", f"Referer: {ref}"])
            ...
            # Direct HTTP download with aria2c - only if yt-dlp is not found
                ...
                if headers:
                    ua = headers.get("User-Agent") or headers.get("user-agent")
                    if ua:
                        aria_cmd.append(f"--user-agent={ua}")
                    ref = headers.get("Referer") or headers.get("referer")
                    if ref:
                        aria_cmd.append(f"--referer={ref}")
            ...
            # Standard requests download - only if all other tools are not found
                ...
                with session.get(
                    url, headers=req_headers, stream=True, timeout=(10, 300)
                ) as r:
                    r.raise_for_status()
                ...
                console.print(
                    "[red]No suitable download tool (yt-dlp, ffmpeg, aria2c) found.[/red]"
                )
                return
        ...
        if success:
            ...
            if (
                embed_subtitles
                and shutil.which("ffmpeg")
                and ("sub_filename" in locals() and os.path.exists(sub_filename))
            ):
                try:
    ...
    console.print(
        Panel(
            f"[green]Download started in background![/green]\n[dim]You can continue using the app while it downloads.[/dim]",
