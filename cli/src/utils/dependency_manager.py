        ...
        if str(archive_path).endswith(".zip"):
            with zipfile.ZipFile(archive_path, 'r') as zip_ref:
                for file in zip_ref.namelist():
                    if file.endswith(f"/{bin_name}") or file.endswith(f"\\{bin_name}") or file == bin_name or \
                       file.endswith(f"/{bin_name}.exe") or file.endswith(f"\\{bin_name}.exe") or file == f"{bin_name}.exe":
...
def bootstrap_dependencies():
    ...
    if not shutil.which("yt-dlp"):
        url = None
        if os_name == "win32": url = "https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp.exe"
        elif os_name == "darwin": url = "https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp_macos"
    ...
    if not shutil.which("aria2c"):
        patterns = ["aria2"]
        if os_name == "win32": patterns.extend(["win", "64bit", ".zip"])
        elif os_name == "darwin": patterns.extend(["osx", ".tar.gz"])
        else: patterns.extend(["linux", "64bit", ".tar.gz"])
        ...
        if url:
            ...
            if _download_file(url, tmp_archive):
                ...
                if ext_path: os.chmod(ext_path, 0o755)
                tmp_archive.unlink(missing_ok=True)

    # 3. ffmpeg (Zip/Tar)
    if not shutil.which("ffmpeg"):
        patterns = ["ffmpeg"]
        if os_name == "win32": patterns.extend(["win64", "gpl", ".zip"])
        elif os_name == "darwin": pass # Hard to find official static builds for mac on GH
        else: patterns.extend(["linux64", "gpl", ".tar.xz"])
        ...
        if url:
            ...
            if _download_file(url, tmp_archive):
                ...
                if f1: os.chmod(f1, 0o755)
                if f2: os.chmod(f2, 0o755)
                tmp_archive.unlink(missing_ok=True)
