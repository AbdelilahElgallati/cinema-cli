            ...
            if process.returncode == 0:
                try:
                    ...
                    try:
                        ...
                        console.print(
                            f"[yellow]Could not embed subtitles: {e}[/yellow]"
                        )
                except Exception:
                    pass
        ...
        try:
            ...
            for ext in [".srt", ".vtt", ".ass", ".sub"]:
                ...
                if os.path.exists(possible):
                    try:
                        shutil.move(
                            possible, os.path.join(dest_dir, os.path.basename(possible))
                        )
                    except Exception:
                        pass
    ...
    def _embed_subtitles(self, task):
        ...
        try:
            ...
            cmd = [
                ...
                sub_codec,
                "-metadata:s:s:0",
                "language=ara",  # Mark as Arabic
                "-disposition:s:0",
                "default",  # Make subtitle default/visible
                "-disposition:v:0",
                "default",  # Keep video as default
                "-y",
                "-loglevel",
                "error",
                temp_output,
