...
class TestPlayerIPC(unittest.TestCase):
    ...
    def test_P1_mpv_launches_before_subtitle_fetch(
        self, mock_clear, mock_console, mock_resolve, mock_prep, mock_thread, mock_popen
    ):
        ...
        ipc_arg = [
            a
            for l in mpv_args
            for a in ([l] if isinstance(l, str) else [])
            if "--input-ipc-server=" in str(a)
        ]
