...
from src.utils.player import _background_subtitle_handler
from src.utils.player import _prepare_subtitles
from src.utils.player import _subtitle_result_to_temp_file
...
class TestSubtitleFallbackFlow(unittest.TestCase):
    def test_fetch_subtitles_uses_subdl_for_missing_langs(self):
        with patch("src.utils.subtitles._get_api_key", return_value="os-key"), patch(
            "src.utils.subtitles._get_subdl_key", return_value="subdl-key"
        ), patch(
            "src.utils.subtitles._fetch_from_opensubtitles", return_value=[]
        ) as os_fetch, patch(
            "src.utils.subtitles.fetch_subtitles_subdl",
            return_value=[{"lang": "en", "ext": "srt", "content": b"1\n00:00:00,000 --> 00:00:01,000\nHello\n"}],
        ) as subdl_fetch:
    ...
    def test_prepare_subtitles_fills_missing_preferred_langs(self):
        ...
        with patch("src.utils.player.requests.get", return_value=_FakeResponse(b"1\n00:00:00,000 --> 00:00:01,000\n\xd9\x85\xd8\xb1\xd8\xad\xd8\xa8\xd8\xa7\n")), patch(
            "src.utils.player.fetch_subtitles", return_value=[fallback_sub]
        ) as fallback_fetch, patch(
            "src.utils.player.load_json_data", return_value={}
        ):
    ...
    def test_download_subtitles_fills_missing_lang_even_with_provider_subs(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir, patch(
            ...
            "src.utils.storage.load_json_data", return_value={}
        ):
    ...
    def test_prepare_subtitles_prefers_arabic_over_fallback_when_available(self):
        def _fake_fetch(title, langs, **kwargs):
            ...
            for lang in langs:
                if lang == "ar":
                    out.append({"lang": "ar", "ext": "srt", "content": b"1\n00:00:00,000 --> 00:00:01,000\n\xd9\x85\xd8\xb1\xd8\xad\xd8\xa8\xd8\xa7\n"})
                if lang == "en":
                    out.append({"lang": "en", "ext": "srt", "content": b"1\n00:00:00,000 --> 00:00:01,000\nHello\n"})
            return out

        with patch("src.utils.player.requests.get", side_effect=Exception("no provider subtitles")), patch(
    ...
    def test_background_handler_skips_fallback_when_arabic_already_present(self):
        with patch("src.utils.player.os.path.exists", return_value=True), patch(
            "src.utils.player.fetch_subtitles"
        ) as fetch_mock:
        ...
        def _fake_fetch(title, langs, **kwargs):
            ...
            for lang in langs:
                ...
                if lang == "en":
                    out.append({"lang": "en", "ext": "srt", "content": b"1\n00:00:00,000 --> 00:00:01,000\nHello\n"})
                if lang == "fr":
                    out.append({"lang": "fr", "ext": "srt", "content": b"1\n00:00:00,000 --> 00:00:01,000\nBonjour\n"})
            return out

        with patch("src.utils.player.requests.get", side_effect=Exception("no provider")), patch(
            "src.utils.player.fetch_subtitles", side_effect=_fake_fetch
        ), patch("src.utils.player.load_json_data", return_value={}):
            paths, langs, _ = _prepare_subtitles(
                subtitles=[],
                headers={},
    ...
    def test_download_subtitles_fetches_each_missing_language_once(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir, patch(
            "src.utils.download_manager.fetch_subtitles", side_effect=_fake_fetch
